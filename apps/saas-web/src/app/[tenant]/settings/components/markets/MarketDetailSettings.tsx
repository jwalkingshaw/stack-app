...
export default function MarketDetailSettings({ tenantSlug, marketId }: MarketDetailSettingsProps) {
  ...
  const availableLocaleCatalog = useMemo(() => {
  ...
  }, [activeLocaleIds, localeById, localeCatalog]);
  ...
  const ensureLocaleByCode = useCallback(
    ...
    [createLocale, localeByCode, localeCatalogByCode]
  );
    ...
    >
      <div className="space-y-6">
            ...
            <section className="space-y-3 rounded-lg border border-border/60 bg-card p-4">
              ...
              <div className="grid gap-2 sm:grid-cols-2">
                ...
                <div className="rounded-md border border-border/60 px-3 py-2 text-sm">
                  ...
                  {market.default_locale_id ? localeById.get(market.default_locale_id)?.code || '-' : '-'}
                </div>
              </div>
            </section>
        ...
        <section className="space-y-3 rounded-lg border border-border/60 bg-card p-4">
          ...
          <div className="grid gap-2 sm:grid-cols-[minmax(0,1fr)_auto]">
            <Select value={addLocaleCode} onValueChange={setAddLocaleCode} disabled={referenceLoading}>
              <SelectTrigger>
            ...
            <Button
              variant="secondary"
              disabled={saving || referenceLoading || !addLocaleCode}
              onClick={() => {
                if (!addLocaleCode) return;
                runAction(
                  async () => {
                    const locale = await ensureLocaleByCode(addLocaleCode);
                    await assignLocale(market.id, locale.id);
                  },
          ...
          <div className="grid gap-2 md:grid-cols-[minmax(0,1fr)_minmax(0,1fr)_auto]">
            ...
            <Button
              variant="outline"
              disabled={saving || referenceLoading || !customLocaleName.trim() || !customLocaleCode.trim()}
              onClick={() => {
                runAction(
                  async () => {
                    ...
                    await assignLocale(market.id, created.id);
                  },
