...
export default function SetDetailSettings({ tenantSlug, setId }: SetDetailSettingsProps) {
  ...
  // Markets containing this saved scope (read-only context)
  ...
  // Readiness tab (product saved scopes only)
            ...
            <div className="rounded-lg border border-border/60 bg-card px-4 py-3">
              <div className="flex items-center justify-between gap-4">
                <div className="min-w-0">
                  <p className="text-sm font-medium text-foreground">Publishing workflow</p>
                  <p className="text-xs text-muted-foreground mt-0.5">
                    Scopes stay reusable access libraries. Use Publishing to send Portal content to partners after the scope and output profile are ready.
                  </p>
                </div>
                <div className="shrink-0">
                  <Link href={`/${tenantSlug}/syndication`}>
                    <Button size="sm">Open Publishing</Button>
                  </Link>
                </div>
              </div>
              <p className="mt-2 text-xs text-muted-foreground">
                {setInfo.output_profile
                  ? `Legacy default profile: ${setInfo.output_profile.name}. Keep it only for compatibility while launch access uses the Portal profile.`
                  : 'No legacy default profile is attached to this scope.'}
              </p>
            </div>
          )}

          {/* Asset saved scope companion note — product saved scopes only */}
          {!isAsset && (
            <div className="rounded-lg border border-border/60 bg-muted/40 px-4 py-3">
              <p className="text-sm font-medium text-foreground">Brand Libraries</p>
              <p className="text-xs text-muted-foreground mt-0.5">
                Pair this Product Scope with Brand Library Scopes when partners need guidelines, compliance certificates, and other standing files alongside published Portal product content.
              </p>
            </div>
          )}
        </div>
        ...
        <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as 'items' | 'grants' | 'rules' | 'readiness')}>
            ...
            {itemsLoading ? (
              ...
              <>
                ...
                <p className="text-sm text-muted-foreground">
                  {isAsset
                    ? 'Add assets and folders to this Brand Library Scope from the Assets view.'
                    : 'Add products and variants to this Product Scope from the Products view.'}
                  {itemCounts.total > 0 ? (
                    <>
                      {' '}
                      <Link
                  ...
                  <div className="space-y-1">
                    {isAsset ? (
                      ...
                      <>
                        ...
                        {/* Orphan variants (parent not in this saved scope) */}
          ...
          {/* ── Access Summary tab ── */}
            ...
            <div className="rounded-lg border border-border/60 bg-card p-4">
              <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                <div>
                  <p className="text-sm font-medium text-foreground">Scopes do not publish on their own</p>
                  <p className="mt-1 text-sm text-muted-foreground">
                    This page defines reusable access context only. Use Publishing to deliver Portal content to partners.
                  </p>
                </div>
                <Link href={`/${tenantSlug}/syndication`}>
                  <Button size="sm">Open Publishing</Button>
              ...
              {setMarketsLoading ? (
                ...
                <>
                  ...
                  <p className="text-xs text-muted-foreground">
                    Markets attached to this scope define which regional content version is valid when partners view it in the Portal.
                  </p>
                </>
              ) : (
                <p className="text-xs text-muted-foreground">
                  This scope is not attached to any markets yet. Add markets when you want this scope to resolve against specific regional locale sets.
                </p>
              )}
            </div>
              ...
              <>
                <p className="text-sm text-muted-foreground">
                  {availablePartners.length === 0
                    ? 'No partner organizations are connected to this workspace yet.'
                    : 'Partner access shown below is read-only here. Use Publishing to deliver this scope through the Portal to specific partner audiences.'}
                </p>
                    ...
                    {grants.map((grant) => (
                      ...
                      >
                        ...
                        <span className="text-xs text-muted-foreground">
                          {grant.updated_at || grant.created_at ? `Updated ${new Date(grant.updated_at || grant.created_at || '').toLocaleDateString()}` : 'Active'}
                        </span>
                      </div>
                    ))}
                  </div>
          ...
          <TabsContent value="rules" className="space-y-4 pt-4">
            ...
            <div className="space-y-3 rounded-lg border border-border/60 bg-card p-4">
              ...
              <p className="text-xs text-muted-foreground">
                Rules automatically include matching {isAsset ? 'assets' : 'products'} in this scope.
              </p>
            ...
            <TabsContent value="readiness" className="space-y-4 pt-4">
              ...
              {readinessLoading || (activeTab === 'readiness' && !hasLoadedReadiness) ? (
                ...
                <div className="rounded-lg border border-border/60 bg-card p-4">
                    ...
                    <p className="text-sm text-muted-foreground">
                      Run readiness from Publishing after choosing the Portal output and delivery target for this scope.
                    </p>
                    <Link href={`/${tenantSlug}/syndication`}>
                      <Button size="sm" variant="outline">Open Publishing</Button>
                    </Link>
                  </div>
                </div>
              ) : (
