...
export function UpdateDetailClient({ tenantSlug, updateId }: UpdateDetailClientProps) {
  ...
  const loadOnboardingShareSetOptions = useCallback(async () => {
    ...
    try {
      ...
      const payload = await response.json().catch(() => ({}));
      if (!response.ok) {
        setOnboardingShareSetError(payload?.error || "Failed to load saved scopes.");
  ...
  const loadShareSettings = useCallback(async () => {
    ...
    try {
      ...
      setOnboardingShareSetIds(
        Array.isArray(payload?.onboardingSavedScopeIds)
          ? payload.onboardingSavedScopeIds
  ...
  const updateShareSettings = async (params: {
    ...
    try {
      ...
      if (Array.isArray(params.onboardingShareSetIds)) {
        ...
        body.onboardingSavedScopeIds = params.onboardingShareSetIds;
      }
  ...
  return (
    <PageContentContainer mode="form" padding="page" className="space-y-4">
                    ...
                    <button
                      ...
                      className={`group flex min-w-0 items-center gap-2 rounded-md border px-2.5 py-2 text-left transition-colors ${
                        index < currentStepIndex
                          ? "border-[var(--color-accent-black)] bg-[var(--color-accent-black)]/10"
                          : activeStep === step
                            ? "border-[var(--color-accent-black)] bg-background"
                            : "border-border bg-background hover:border-[var(--color-accent-black)]/40"
                      }`}
                      ...
                      <span
                        className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-full border text-xs font-semibold transition-colors ${
                          index < currentStepIndex
                            ...
                            : activeStep === step
                              ? "border-[var(--color-accent-black)] bg-background text-[var(--color-accent-black)]"
                              ...
                              <div className="space-y-0.5">
                                {productPickerRows.map((row) => {
                                  ...
                                  return (
                                    <div
                                      key={product.id}
                                      className={`flex cursor-pointer items-center gap-3 rounded-md px-2 py-1.5 transition-colors ${row.depth === 1 ? "ml-5" : ""} ${checked ? "bg-[var(--color-accent-black)]/10" : "hover:bg-muted/40"}`}
                                  ...
                                  {filteredAssetPickerRows.map((asset) => {
                                    ...
                                    return (
                                      <button
                                        ...
                                        className={`relative aspect-square overflow-hidden rounded-lg border-2 transition-all ${checked ? "border-[var(--color-accent-black)] ring-2 ring-[var(--color-accent-black)]/30" : "border-border hover:border-muted-foreground"}`}
                                      >
                                        {isImageAsset(asset) ? (
                                        ...
                                        {checked && (
                                          <div className="absolute inset-0 flex items-center justify-center bg-[var(--color-accent-black)]/20">
                                            <div className="rounded-full bg-[var(--color-accent-black)] p-0.5">
                            ...
                            <div className="flex items-center justify-between border-t border-gray-200 px-3 py-2.5">
                              <div className="flex items-center gap-2">
                                ...
                                {filteredAssetPickerRows.length > 0 && selectedAssetIds.size < filteredAssetPickerRows.length ? (
                                  <button
                                    type="button"
                                    className="text-xs text-[var(--color-accent-black)] hover:underline"
                      ...
                      <div
                        ...
                        className={`flex items-center gap-3 rounded-lg border px-3 py-2.5 transition-colors ${
                          dragOverKitItemId === item.id
                            ? "border-[var(--color-accent-black)] bg-[var(--color-accent-black)]/5"
                                ...
                                return (
                                  <label
                                    ...
                                    className={`flex cursor-pointer items-center gap-3 rounded border px-3 py-2 ${
                                      checked ? "border-[var(--color-accent-black)] bg-muted/40" : "border-border"
                                    }`}
                                  >
                                    <input
                    ...
                    {([
                      ...
                      <button
                        ...
                        className={`flex flex-col gap-2 rounded-lg border p-4 text-left transition-colors ${
                          deliveryMode === mode
                            ? "border-[var(--color-accent-black)] bg-[var(--color-accent-black)]/5 text-foreground"
                            : "border-border bg-background text-muted-foreground hover:border-[var(--color-accent-black)]/40 hover:text-foreground"
                        }`}
                        onClick={() => setDeliveryMode(mode)}
                      >
                        <span className={deliveryMode === mode ? "text-[var(--color-accent-black)]" : "text-muted-foreground"}>
                          {icon}
                        </span>
                        <span>
                      ...
                      {shareAvailable ? (
                        <div className="rounded-md border border-border p-3">
                          ...
                          <div className="mt-3 rounded-md border border-border p-3">
                            <p className="text-xs font-medium text-foreground">Onboarding Saved Scopes</p>
                            <p className="mt-1 text-xs text-muted-foreground">
                              Applied only to new partners created via this link.
                            </p>

                            {onboardingShareSetLoading ? (
                              <p className="mt-2 text-xs text-muted-foreground">Loading Saved Scopes...</p>
                            ) : onboardingShareSetOptions.length === 0 ? (
                              <p className="mt-2 text-xs text-muted-foreground">
                                No Saved Scopes available. Create one in Settings first.
                              </p>
                            ) : (
      ...
      {kitDrawerOpen ? (
        <button
          type="button"
          className="fixed bottom-6 right-6 z-50 flex h-11 w-11 items-center justify-center rounded-full bg-[var(--color-accent-black)] text-white shadow-xl transition-all hover:bg-[var(--color-accent-black-hover)] active:scale-95 active:bg-[var(--color-accent-black-active)]"
        ...
        <button
          type="button"
          className="fixed bottom-6 right-6 z-40 flex items-center gap-2 rounded-full bg-[var(--color-accent-black)] px-4 py-2.5 text-sm font-medium text-white shadow-xl transition-all hover:bg-[var(--color-accent-black-hover)] active:scale-95 active:bg-[var(--color-accent-black-active)]"
