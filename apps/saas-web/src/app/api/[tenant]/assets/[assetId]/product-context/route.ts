...
export async function GET(
  ...
  try {
    ...
    const contextStats = linkRows.reduce<Record<string, ContextStat>>((acc, link) => {
      const contextKey = link.link_context || "unclassified";
      const confidence = typeof link.confidence === "number" ? link.confidence : 0;
      if (!acc[contextKey]) {
        acc[contextKey] = { count: 0, avgConfidence: 0, total: 0 };
      }
      acc[contextKey].count++;
      acc[contextKey].total += confidence;
      acc[contextKey].avgConfidence =
        acc[contextKey].total / acc[contextKey].count;
      return acc;
    }, {});

    const formattedLinks = linkRows.map((link) => {
      const productSource = Array.isArray(link.products) ? link.products[0] : link.products;
      const product =
      ...
      return {
        ...
        productName: typeof product.product_name === "string" ? product.product_name : "",
