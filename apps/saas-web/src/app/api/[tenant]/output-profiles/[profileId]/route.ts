...
async function resolveProfile(organizationId: string, profileId: string) {
  const { data, error } = await getSupabaseServer()
    ...
    .select(`
      ...
      attribute_mappings:output_profile_attribute_mappings(id, attribute_code, attribute_label, source_mode, source_field_code, override_field_code, source_slot_code, constant_value, resolution_rule, is_required, max_length, notes, sort_order, metadata),
      slot_definitions:output_slot_definitions(id, slot_code, slot_name, asset_kind, document_type, certificate_type, label_panel_type, classification, is_required, allow_multiple, sort_order, metadata)
    `)
    .eq("id", profileId)
