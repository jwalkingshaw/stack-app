...
export function normalizeRecipientSelection(input: unknown): RecipientSelection {
  ...
  return {
    ...
    shareSetIds: normalizeUuidArray(
      payload.savedScopeIds ||
        payload.saved_scope_ids ||
        payload.shareSetIds ||
        payload.share_set_ids
    ),
  ...
  if (setsError) {
    if (isMissingTableError(setsError)) {
      return {
        ok: false,
        status: 503,
        error: "Saved scope tables are unavailable. Apply saved scope migrations first.",
      };
    }
  ...
  if (grantsError) {
    if (isMissingTableError(grantsError)) {
      return {
        ok: false,
        status: 503,
        error: "Saved scope grant table is unavailable. Apply saved scope migrations first.",
      };
    }
    return { ok: false, status: 500, error: "Failed to resolve partner saved scope grants" };
  }
...
function formatBlockedRecipientAccessError(params: {
  ...
  return [
    `${blockedCount} selected partner${blockedCount === 1 ? "" : "s"} cannot access all Kit items via current saved scope grants.`,
    "Assign or adjust saved scopes for those partners before sending this update.",
  ...
  for (const partnerOrganizationId of partnerOrganizationIds) {
    ...
    if (!grantedProducts.foundationAvailable || !grantedAssets.foundationAvailable) {
      return {
        ...
        status: 503,
        error:
          "Saved scope visibility foundation is unavailable. Apply latest sharing migrations first.",
        blockedRecipients: [],
      };
    }
