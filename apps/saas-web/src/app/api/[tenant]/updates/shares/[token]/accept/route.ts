...
async function applyOnboardingShareSetGrants(params: {
  ...
  if (shareSetError) {
    ...
    return { ok: false, status: 500, error: "Failed to validate onboarding share sets" };
  }
  ...
  if (existingError) {
    if (existingError.code === "42P01" || existingError.code === "PGRST205") {
      return {
        ok: false,
        status: 503,
        error: "Share set grant table is unavailable. Apply database migrations first.",
      };
    }
    return {
      ok: false,
      status: 500,
      error: "Failed to resolve existing onboarding share set grants",
    };
  }
  ...
  if (insertError) {
    ...
    return {
      ok: false,
      status: 500,
      error: "Failed to apply onboarding share set grants",
    };
  }
