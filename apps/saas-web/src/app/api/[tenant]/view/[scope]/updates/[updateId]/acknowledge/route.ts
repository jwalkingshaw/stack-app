...
// POST /api/[tenant]/view/[scope]/updates/[updateId]/acknowledge
export async function POST(
  ...
  try {
    ...
    const nextStatus =
      recipient.status === "activated" ? "activated" : "acknowledged";
    ...
    if (error) {
      console.error("Failed to acknowledge partner update:", error);
    ...
    await logSecurityEvent(getSupabaseServer(), {
      organizationId: recipient.organizationId,
      actorUserId: scopeAccess.userId,
      action: "partner_update.acknowledge",
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        activatedAt: data?.activated_at || recipient.activatedAt || null,
        updatedAt: data?.updated_at || nowIso,
      },
    });
  } catch (error) {
    console.error("Error in acknowledge POST:", error);
