...
export function MarketContextProvider({ tenantSlug, children }: MarketContextProviderProps) {
  ...
  useEffect(() => {
    ...
    if (markets.length > 0) {
      ...
      if (byUrlCode) {
        if (selectedMarketId !== byUrlCode.id) {
          setSelectedMarketIdState(byUrlCode.id)
        }
        return
      }
      ...
      if (!hasSelected) {
        ...
        setSelectedMarketIdState(byCode?.id || defaultMarket?.id || markets[0].id)
      }
  ...
  useEffect(() => {
    ...
    const byMarketDefault =
      selectedMarket?.default_locale_id && allowedLocaleIds.includes(selectedMarket.default_locale_id)
        ? selectedMarket.default_locale_id
        : null
    const urlLocaleCode = (searchParams.get('locale') || '').trim().toLowerCase()
    const byUrlCode = urlLocaleCode
      ? locales.find(
          (locale) =>
            locale.code.toLowerCase() === urlLocaleCode && allowedLocaleIds.includes(locale.id)
        )
      : null

    if (urlLocaleCode) {
      const fallbackLocaleId = byUrlCode?.id || byMarketDefault || allowedLocaleIds[0]
      if (selectedLocaleId !== fallbackLocaleId) {
        setSelectedLocaleIdState(fallbackLocaleId)
      }
      return
    }
    ...
    if (!hasSelected) {
      const byCode = persistedCodes.localeCode
        ...
        : null
      setSelectedLocaleIdState(byMarketDefault || byCode?.id || allowedLocaleIds[0])
    }
  }, [
    searchParams,
    hasHydratedSelection,
